    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['hbzy:hgshbzy:remove']"
        >删除
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['hbzy:hgshbzy:export']"
        >导出
        </el-button>
      </el-col>
    </el-row>

    <el-table v-loading="loading" :data="hgshbzyList" @selection-change="handleSelectionChange" border>
...
<!--      <el-table-column label="潜力来源" align="center" prop="glly"/>-->
<!--      <el-table-column label="是否立项" align="center" prop="sflx"/>-->
...
<!--      <el-table-column label="备注" align="center" prop="bz"/>
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px">
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-row>
          <el-col :span="11">
            <el-form-item label="是否立项" prop="sflx">
              <el-input v-model="form.sflx" placeholder="请输入是否立项"/>
            </el-form-item>
          </el-col>
          <el-col :span="11">
...
<script>
import { listHgshbzy, getHgshbzy, delHgshbzy, addHgshbzy, updateHgshbzy, exportHgshbzy } from '@/api/hbzy/hgshbzy'
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listHgshbzy(this.queryParams).then(response => {
        this.hgshbzyList = response.rows
    ...
    handleAdd() {
      this.reset()
      this.open = true
      this.title = '添加旱改水后备资源'
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset()
      const bsm = row.bsm || this.ids
      getHgshbzy(bsm).then(response => {
        this.form = response.data
        this.open = true
        this.title = '修改旱改水后备资源'
      })
    },
    /** 提交按钮 */
    submitForm: function() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.bsm != undefined) {
            updateHgshbzy(this.form).then(response => {
            ...
            addHgshbzy(this.form).then(response => {
              if (response.code === 200) {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除旱改水后备资源编号为"' + bsms + '"的数据项?', '警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(function() {
        return delHgshbzy(bsms)
      }).then(() => {
        this.getList()
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有旱改水后备资源数据项?', '警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(function() {
        return exportHgshbzy(queryParams)
      }).then(response => {
